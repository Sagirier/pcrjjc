#Created by ColdThunder11 2021/1/3
#changed by lulu to another api 2021/1/7
from os import path
import json
from hoshino import Service, priv
import nonebot
from hoshino.typing import NoticeSession
import asyncio
from .queryapi import getprofile
import copy

sv_help = '''
[竞技场绑定 uid] 绑定竞技场排名变动推送（仅下降），默认双场均启用
[竞技场查询( uid)] 查询竞技场简要信息
[停止竞技场订阅] 停止战斗竞技场排名变动推送
[停止公主竞技场订阅] 停止公主竞技场排名变动推送
[启用竞技场订阅] 启用战斗竞技场排名变动推送
[启用公主竞技场订阅] 启用公主竞技场排名变动推送
[删除竞技场订阅] 删除竞技场排名变动推送绑定
[竞技场订阅状态] 查看排名变动推送绑定状态
'''.strip()

sv = Service('竞技场推送',help_=sv_help, bundle='pcr查询')
svgx = Service('竞技场排名更新')

@sv.on_fullmatch('jjc帮助', only_to_me=False)
async def send_jjchelp(bot, ev):
    await bot.send(ev, sv_help)

def Init():
    config_path = path.join(path.dirname(__file__),"binds.json")
    with open(config_path,"r",encoding="utf8")as fp:
        binds = json.load(fp)
    return binds

def save_binds(binds):
    config_path = path.join(path.dirname(__file__),"binds.json")
    jsonStr = json.dumps(binds, indent=4)
    with open(config_path,"r+",encoding="utf8")as fp:
        fp.truncate(0)
        fp.seek(0)
        fp.write(jsonStr)

@sv.on_rex(r'竞技场绑定 (.{0,15})$')
async def on_arena_bind(bot,ev):
    binds = Init()
    robj = ev['match']
    id = robj.group(1)
    if not id.isdigit() or not len(id) == 13:
        await bot.send(ev,"ID格式错误，请检查",at_sender=True)
        return
    uid = str(ev['user_id'])
    gid = str(ev['group_id'])
    if not uid in binds["arena_bind"]:
        binds["arena_bind"][uid] = {"id":id,"uid":uid,"gid":gid,"arena_on":True,"grand_arena_on":True,"origin_arena_rank": 0,"origin_grand_arena_rank": 0,"arena_rank": 0,"grand_arena_rank": 0}
    else:
        binds["arena_bind"][uid]["id"] = id
        binds["arena_bind"][uid]["uid"] = uid
        binds["arena_bind"][uid]["gid"] = gid
    save_binds(binds)
    await bot.send(ev,"竞技场绑定成功",at_sender=True)

@sv.on_rex(r'(竞技场查询 (.{0,15})$)|(^竞技场查询$)')
async def on_query_arena(bot,ev):
    binds = Init()
    robj = ev['match']
    try:
        id = robj.group(2)
    except:
        id = ""
    if id=='' or id==None:
        uid = str(ev['user_id'])
        if not uid in binds["arena_bind"]:
            await bot.send(ev,"您还未绑定竞技场",at_sender=True)
            return
        else:
            id = binds["arena_bind"][uid]["id"]
    if not id.isdigit() or not len(id) == 13:
        await bot.send(ev,"ID格式错误，请检查",at_sender=True)
        return
    try:
        res = await getprofile(int(id))
        res = res["user_info"]
        '''if res["err_code"] == 403:
            sv.logger.info("您的API KEY错误或者被屏蔽，请尽快停止本插件")
            await bot.send(ev,"错误403，查询出错，请联系维护者",at_sender=True)
            return'''
        if res == "queue":
            sv.logger.info("成功添加至队列"),
            await bot.send(ev,"请等待源站更新数据，稍等几分钟再来查询",at_sender=True)
        if res == "id err":
            sv.logger.info("该viewer_id有误")
            await bot.send(ev,"查询出错，请检查ID是否正确",at_sender=True)
            return
        strList = []
        strList.append("\n")
        strList.append("竞技场排名：")
        strList.append(str(res["arena_rank"]))
        strList.append("\n")
        strList.append("公主竞技场排名：")
        strList.append(str(res["grand_arena_rank"]))
        await bot.send(ev,"".join(strList),at_sender=True)
    except:
        await bot.send(ev,"查询出错，请检查ID是否正确",at_sender=True)
    pass

@sv.on_fullmatch('停止竞技场订阅')
async def disable_arena_sub(bot,ev):
    binds = Init()
    uid = str(ev['user_id'])
    if not uid in binds["arena_bind"]:
        await bot.send(ev,"您还未绑定竞技场",at_sender=True)
    else:
        binds["arena_bind"][uid]["arena_on"] = False
        save_binds(binds)
        await bot.send(ev,"停止竞技场订阅成功",at_sender=True)

@sv.on_fullmatch('停止公主竞技场订阅')
async def disable_grand_arena_sub(bot,ev):
    binds = Init()
    uid = str(ev['user_id'])
    if not uid in binds["arena_bind"]:
        await bot.send(ev,"您还未绑定竞技场",at_sender=True)
    else:
        binds["arena_bind"][uid]["grand_arena_on"] = False
        save_binds(binds)
        await bot.send(ev,"停止公主竞技场订阅成功",at_sender=True)

@sv.on_fullmatch('启用竞技场订阅')
async def enable_arena_sub(bot,ev):
    binds = Init()
    uid = str(ev['user_id'])
    if not uid in binds["arena_bind"]:
        await bot.send(ev,"您还未绑定竞技场",at_sender=True)
    else:
        binds["arena_bind"][uid]["arena_on"] = True
        save_binds(binds)
        await bot.send(ev,"启用竞技场订阅成功",at_sender=True)

@sv.on_fullmatch('启用公主竞技场订阅')
async def enable_arena_sub(bot,ev):
    binds = Init()
    uid = str(ev['user_id'])
    if not uid in binds["arena_bind"]:
        await bot.send(ev,"您还未绑定竞技场",at_sender=True)
    else:
        binds["arena_bind"][uid]["grand_arena_on"] = True
        save_binds(binds)
        await bot.send(ev,"启用公主竞技场订阅成功",at_sender=True)

@sv.on_prefix('删除竞技场订阅')
async def delete_arena_sub(bot,ev):
    binds = Init()
    if len(ev.message) == 1 and ev.message[0].type == 'text' and not ev.message[0].data['text']:
        uid = str(ev['user_id'])
        if not uid in binds["arena_bind"]:
            await bot.finish(ev, "您还未绑定竞技场", at_sender=True)
        else:
            binds["arena_bind"].pop(uid)
            save_binds(binds)
            await bot.send(ev, "删除竞技场订阅成功", at_sender=True)
    elif ev.message[0].type == 'at':
        if not priv.check_priv(ev, priv.SUPERUSER):
            await bot.finish(ev, '删除他人订阅请联系维护', at_sender=True)
        else:
            uid = str(ev.message[0].data['qq'])
            if not uid in binds["arena_bind"]:
                await bot.finish(ev, "对方尚未绑定竞技场", at_sender=True)
            else:
                binds["arena_bind"].pop(uid)
                save_binds(binds)
                await bot.send(ev, "删除竞技场订阅成功", at_sender=True)
    else:
        await bot.finish(ev, '参数格式错误, 请重试')

@sv.on_fullmatch('竞技场订阅状态')
async def send_arena_sub_status(bot,ev):
    binds = Init()
    uid = str(ev['user_id'])
    if not uid in binds["arena_bind"]:
        await bot.send(ev,"您还未绑定竞技场",at_sender=True)
    else:
        strList = []
        strList.append("当前竞技场绑定ID：")
        strList.append(str(binds["arena_bind"][uid]["id"]))
        strList.append("\n竞技场订阅：")
        if binds["arena_bind"][uid]["arena_on"]:
            strList.append("开启")
        else:
            strList.append("关闭")
        strList.append("\n公主竞技场订阅：")
        if binds["arena_bind"][uid]["grand_arena_on"]:
            strList.append("开启")
        else:
            strList.append("关闭")
        await bot.send(ev,"".join(strList),at_sender=True)

@svgx.scheduled_job('cron', minute='*')
async def on_arenarank_schedule():
    binds = Init()
    arena_bind = binds["arena_bind"]
    for user in arena_bind:
        user = str(user)
        try:
            id = arena_bind[user]["id"]
            await asyncio.sleep(1.5)
            result = await getprofile(int(id))
            res = result["user_info"]
            if res == "queue" or res == "id err":
                binds["arena_bind"][user]["query_statu"] = False
                svgx.logger.info(f"对{id}的检查出错：{res}")
            else:
                binds["arena_bind"][user]["origin_arena_rank"] = arena_bind[user]["arena_rank"]
                binds["arena_bind"][user]["origin_grand_arena_rank"] = arena_bind[user]["grand_arena_rank"]
                binds["arena_bind"][user]["arena_rank"] = res["arena_rank"]
                binds["arena_bind"][user]["grand_arena_rank"] = res["grand_arena_rank"]
                binds["arena_bind"][user]["query_statu"] = True
        except:
            binds["arena_bind"][user]["query_statu"] = False
            id = arena_bind[user]["id"]
            svgx.logger.info(f"对{id}的检查出错")
    save_binds(binds)

@sv.scheduled_job('cron', minute="*", second = 30)
async def on_arena_schedule():
    bot = nonebot.get_bot()
    binds = Init()
    arena_bind = binds["arena_bind"]
    for user in arena_bind:
        try:
            user = str(user)
            if arena_bind[user]["arena_on"] and arena_bind[user]["query_statu"]:
                origin_rank = arena_bind[user]["origin_arena_rank"]
                new_rank = arena_bind[user]["arena_rank"]
                if origin_rank>0 and new_rank>0 and origin_rank < new_rank:  # 排名下降
                    msg = "[CQ:at,qq={uid}]您的竞技场排名发生变化：{origin_rank}->{new_rank}，↓{low_rank}".format(uid=arena_bind[user]["uid"], origin_rank=str(origin_rank), new_rank=str(new_rank),low_rank=str(int(new_rank) - int(origin_rank)))
                    await bot.send_group_msg(group_id=int(arena_bind[user]["gid"]), message=msg)
            if arena_bind[user]["grand_arena_on"] and arena_bind[user]["query_statu"]:
                origin_rank = arena_bind[user]["origin_grand_arena_rank"]
                new_rank = arena_bind[user]["grand_arena_rank"]
                if origin_rank>0 and new_rank>0 and origin_rank < new_rank:  # 排名下降
                    msg = "[CQ:at,qq={uid}]您的公主竞技场排名发生变化：{origin_rank}->{new_rank}，↓{low_rank}".format(uid=arena_bind[user]["uid"], origin_rank=str(origin_rank), new_rank=str(new_rank),low_rank=str(int(new_rank) - int(origin_rank)))
                    await bot.send_group_msg(group_id=int(arena_bind[user]["gid"]), message=msg)
        except Exception as e:
            id = arena_bind[user]["id"]
            sv.logger.info(f"对{id}的检查出错:{e}")
            msg = "[CQ:at,qq={uid}]无法查询您的竞技场排名，请确认游戏ID：{id} 是否正确！\n请确认ID正确后发送[删除竞技场订阅] -> [竞技场绑定 id] 给bot重新绑定(>^ω^<)".format(uid=arena_bind[user]["uid"], id=str(id))
            await bot.send_group_msg(group_id=int(arena_bind[user]["gid"]), message=msg)

@sv.on_notice('group_decrease.leave')
async def leave_notice(session: NoticeSession):
    binds = Init()
    uid = str(session.ctx['user_id'])
    if not uid in binds["arena_bind"]:
        pass
    else:
        binds["arena_bind"].pop(uid)
        save_binds(binds)
        pass
    return
